...
class mc_dte_utilidades(osv.osv):
    ...
    def fecha_cadena(self, fecha_texto):
        try:
            ...
            res = nombre_dia + ', ' + str(dia) + ' de ' + nombre_mes + ' de ' + str(anio)
        except:
            res = ''
        return res
    ...
    def dicc_imp(self, cr, uid, inv, context=None):
        inv_obj = self.pool.get('account.invoice')
        inv_brw = inv_obj.browse(cr, uid, inv.id)
        dte_obj = inv_brw.mc_dte_id
        dte_config_obj = inv.journal_id.mc_dte_config_id
        ...
        res['receptor_rs'] = inv_brw.partner_id.name
        res['receptor_rut'] = self.rut_formato_impresion(soup.Encabezado.Receptor.RUTRecep.string.replace('.','',2))
        res['receptor_giro'] = inv_brw.partner_id.giro_cliente
        direccion_comuna_receptor = self.direccion_receptor(cr, uid, inv_brw.partner_id)
        res['receptor_direccion'] = direccion_comuna_receptor['direccion']
        res['receptor_comuna'] = direccion_comuna_receptor['comuna']
        res['cond_pago'] = inv_brw.payment_term and inv_brw.payment_term.name[:22] or ''
