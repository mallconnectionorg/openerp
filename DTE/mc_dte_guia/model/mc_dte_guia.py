...
class stock_picking(osv.osv):
    ...
    def generar_dicc_dte(self, cr, uid, picking, context=None):
        ...
        for linea in lineas_venta:
            ...
            linea_dicc['QtyItem'] = linea.product_uom_qty
    ...
    def generar_guia(self, cr, uid, ids, context=None):
        logger = logging.getLogger('generar_guia')
        ...
        if picking.sale_id:
            ...
            if diario and diario.mc_dte_config_id and diario.sii_dte:
                if diario.mc_dte_config_id.enviar_num_folio:
                    folio = self.pool.get('ir.sequence').get(cr, uid, 'guia.despacho.electronica')
                    if folio:
                        self.write(cr, uid, [picking.id], {'numero_guia': str(folio)})
                        picking = self.browse(cr, uid, picking.id)
                logger.warn('si todo')
                ...
                if mc_dte_id != 0:
                    swvals = dict()
                    swvals['mc_dte_id'] = mc_dte_id
                    mc_dte = dte_obj.browse(cr, uid, mc_dte_id)
                    if not picking.numero_guia:
                        swvals['numero_guia'] = str(mc_dte.folio_dte_sii)
                    self.write(cr, uid, [picking.id], swvals)
    ...
    def consultar_estado_dte(self, cr, uid, ids, context=None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            if picking.mc_dte_id:
                mc_dte_config_id = picking.stock_journal_id.mc_dte_config_id
                detalle = dte_obj.consultar_estado(cr, uid, [picking.mc_dte_id.id], mc_dte_config_id, context=context)
                self.message_post(cr, uid, [picking.id], body=tools.ustr(detalle), context=context)
        return True
